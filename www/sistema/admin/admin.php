...
***** APLICAÇÃO
***** Guia Doméstico
***** Arquivo: /sistema/admin/admin.php
***** Conteúdo: Cadastro de Administradores
...
$ok = true;
$pagina_titulo = "Cadastro de Administradores do Portal";
...
tusuario
where 
tusuario.tipo_usuario = 'ADMINISTRADOR' and 
...
while ($tusuario = $qusuario->fetch(PDO::FETCH_ASSOC)) {
  ...
  $etq .= '                <td data-id_dbg="' . $chave_usuario . '" class="SELECIONA_LINHA">' . $tipo_usuario . '</td>' . "\n";
  $etq .= '                <td data-id_dbg="' . $chave_usuario . '" class="SELECIONA_LINHA text-center">' . $sit_usuario . '</td>' . "\n";
  $etq .= '                <td class="text-center"><a class="link-padrao" href="#" data-bs-toggle="modal" data-bs-target="#MODAL_DIARIO_EXC" id="SHOW_DIARIO_EXC" data-id="' . $chave_usuario . '" data-campo-id="chave_usuario" data-tabela-id="tusuario" data-url-id="' . $Raiz . '" data-caixa-id="caixa_usuario"><i class="fa-regular fa-trash-can" title="Excluir"></a></td>' . "\n";
...
$html_pesq .= '      <form class="row row-cols-lg-auto float-end" action="admin.php" id="FUSUARIO_FILTRO" name="FUSUARIO_FILTRO" method="POST">' . "\n"; // Pesquisa
...
// Corpo
$html .= '      <h5 class="Texto-Titulo">Cadastro de Administradores do Portal</h5>' . "\n";
$html .= '      <div class="card shadow">' . "\n";
$html .= '        <div class="card-header m-0">' . "\n";
$html .= '          ' . $html_pesq . "\n";
$html .= '        </div>' . "\n";
$html .= '        <form action="admin_edita.php" id="FUSUARIO" name="FUSUARIO" method="POST">' . "\n";
...
if ($etq != "") {
  ...
  $html .= '              <tr>' . "\n";
  $html .= '                <th><a href="' . $Raiz . 'sistema/admin/admin.php?ORDEM=CHAVE_USUARIO">Chave</a></th>' . "\n";
  $html .= '                <th><a href="' . $Raiz . 'sistema/admin/admin.php?ORDEM=NOME_USUARIO">Nome</a></th>' . "\n";
  $html .= '                <th><a href="' . $Raiz . 'sistema/admin/admin.php?ORDEM=EMAIL_USUARIO">E-mail</a></th>' . "\n";
  $html .= '                <th><a href="' . $Raiz . 'sistema/admin/admin.php?ORDEM=EMAIL_USUARIO">Cadastro</a></th>' . "\n";  
  $html .= '                <th><a href="' . $Raiz . 'sistema/admin/admin.php?ORDEM=TIPO_USUARIO">Tipo</a></th>' . "\n";
  $html .= '                <th class="text-center"><a href="' . $Raiz . 'sistema/admin/admin.php?ORDEM=SIT_USUARIO">Situação</a></th>' . "\n";
  $html .= '                <th><a href="' . $Raiz . 'sistema/admin/admin.php?ORDEM=CAIXA_USUARIO"> </a></th>' . "\n";
  $html .= '                <th></th>' . "\n";
  $html .= '              </tr>' . "\n";
  $html .= '            </thead>' . "\n";
  $html .= '            <tbody>' . "\n";
...
if ($qtdpagina > 0 && ($qtdreg > $qtdlimite)) {
  ...
  $html .= '                <li class="page-item"><a class="page-link" href="admin.php?PG=FIRST' . (($_SESSION['SEARCHSTRING_USUARIO'] != "") ? '&PROCURAR=' . $_SESSION['SEARCHSTRING_USUARIO'] : "") . '">&laquo;</a></li>' . "\n";
  if ($qtdpagina > 0) {
    $cont_apagina = count($apagina);
    for ($i = 0; $i <= $cont_apagina - 1; $i++) {
      if (($i + 1) >= $paginainicial && ($i + 1) <= $paginafinal) {
        if (($i + 1) == intval($pagina)) {
          $html .= '                <li class="page-item active"><a class="page-link" href="admin.php?PG=' . $apagina[$i] . (($_SESSION['SEARCHSTRING_USUARIO'] != "") ? '&PROCURAR=' . $_SESSION['SEARCHSTRING_USUARIO'] : "") . '">' . $apagina[$i] . '</a></li>' . "\n";
        }
        else {
          $html .= '                <li class="page-item"><a class="page-link" href="admin.php?PG=' . $apagina[$i] . (($_SESSION['SEARCHSTRING_USUARIO'] != "") ? '&PROCURAR=' . $_SESSION['SEARCHSTRING_USUARIO'] : "") . '">' . $apagina[$i] . '</a></li>' . "\n";
        }
      }
    }
  }
  $html .= '                <li class="page-item"><a class="page-link" href="admin.php?PG=LAST' . (($_SESSION['SEARCHSTRING_USUARIO'] != "") ? '&PROCURAR=' . $_SESSION['SEARCHSTRING_USUARIO'] : "") . '" aria-label="Último"><span aria-hidden="true">&raquo;</span></a></li>' . "\n";
  $html .= '              </ul>' . "\n";
  $html .= '            </nav>' . "\n";
  $html .= '          </div>' . "\n";
}
...
<head>
<title>Guia Doméstico - Cadastro de Administradores do Portal</title>
...
<script type="text/javascript">
  function caixa_usuario(ccaixa) {
    ...
    //	parent.location='<?php echo $Raiz; ?>sistema/admin/admin.php?CAIXA_USUARIO=' + vcaixa;
  }
